...
<p>
            sparse is used to build a sparse matrix. Only non-zero entries are stored.</p>

<p>If Mis a full matrix, sparse converts it to a sparse matrix representation, removing all zero values.</p>

<p>If nz is not specified, sparse uses as default value: nz = max([numel(i), numel(j), numel(v)])</p>
