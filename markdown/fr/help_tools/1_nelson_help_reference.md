...
2. Préférez des lignes de résumé courtes pour <code><short_description></code>.
3. Placez les exemples exécutables à l'intérieur de <code><examples></code> en utilisant <code><example_item_data></code> et définissez <code>runnable="cli"</code> si applicable ou <code>runnable="false"</code> (par défaut).
4. Enveloppez le code source de l'exemple dans CDATA pour éviter l'échappement (voir les exemples ci-dessous).
5. Utilisez <code><link linkend="{module}name"></code> pour les références qualifiées par module ; sinon, utilisez des noms simples.
